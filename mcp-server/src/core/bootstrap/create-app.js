...
import { validateSecurityConfigOrExit, validateSecurityAfterPluginsOrExit } from "../security/validate-security-config.js";
...
} from "../authorization/filter-visible-tools.js";
...
export async function createServer() {
  ...
  app.get("/plugins", skipForHtmlNavigation, requireScope("read"), async (req, res) => {
    await emitRestDiscoveryRequested(req, DiscoverySurfaces.REST_PLUGINS_LIST).catch(() => {});
    ...
    await emitRestDiscoveryFiltered(req, DiscoverySurfaces.REST_PLUGINS_LIST, {
      totalCandidates,
      visibleCount,
    }).catch(() => {});
    res.json(plugins);
  ...
  app.get("/tools", skipForHtmlNavigation, requireScope("read"), (req, res) => {
    ...
    const filterOpts = filterOptionsFromContext({
      workspaceId: req.workspaceId,
      tenantId: req.tenantId,
      scopes: req.securityContext?.scopes,
      user: req.securityContext?.user,
    });
    let tools = filterVisibleTools(listTools(), filterOpts);
    if (plugin) {
  ...
  app.post("/ui/token", async (req, res) => {
    if (process.env.NODE_ENV === "production") {
      return res.status(403).json({
        ...
        error: {
          code: "forbidden",
          message: "UI token issuance is disabled in production. Use /auth/login.",
        },
        meta: { requestId: req.requestId },
      });
    }
    ...
    const defaultTtl = silent ? 45 * 60 * 1000 : 5 * 60 * 1000;
  ...
  try {
    const { rehydrateBrainMemoriesFromDb } = await import("../../plugins/brain/brain.memory.js");
    const { loaded } = await rehydrateBrainMemoriesFromDb();
    if (loaded > 0) console.log(`[brain] Rehydrated ${loaded} memories from MSSQL`);
  } catch (err) {
    console.warn("[brain] Memory rehydrate skipped:", err.message);
  }
  try {
    initMasterKey();
  } catch (err) {
  ...
  try {
    await bootstrapSchedulersAndConnectors(listConnectors);
  } catch (err) {
    console.warn("[bootstrap] Schedulers/connectors startup failed:", err.message);
  }
